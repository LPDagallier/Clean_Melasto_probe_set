...
############      SLURM CONFIGURATION      ###################
#SBATCH --job-name=hybpiper2_assemble_Probe_set_comparison_bwa_old
...
step_ID="hybpiper2_assemble_no_stiched_bwa_old"

# - Samples: Tibouchina and Memecylon samples from Jantzen et al 2020 (NCBI BioProject numbers PRJNA573947 and PRJNA576018)
# - Target file used: Old_probe_set_384.fasta
# - HybPiper version 2.1.1
# - BWA

# REFERENCE FILE:
path_to_ref=$path_to_wd/original_template_sequences/Template_sequences;
reference_fasta_file="Old_probe_set_384.fasta"
...
while read name;
do hybpiper assemble -t_dna $reference_fasta_file -r $path_to_fastq/$name"_"*.fastq --prefix $name --cpu 8 --bwa --unpaired $path_to_fastq/$name.fastq --no_stitched_contig;
done < namelist.txt
